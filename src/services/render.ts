...
import {
  ...
  summary,
  verification
} from "./firstupEmployeeCommAuditService.js";

function layout(title: string, active: string, body: string) {
  const nav = [
    { href: "/", label: "Overview" },
    { href: "/comms-lane", label: "Comms Lane" },
    { href: "/message-gaps", label: "Message Gaps" },
    { href: "/send-posture", label: "Send Posture" },
...
<html lang="en">
  ...
  <body>
    ...
    <div class="wrap">
      <div class="topbar">
        <div class="left">Kinetic Gain · FirstUp Employee Comm Audit</div>
        <div class="right">
          <div>synthetic campaign packets · send posture</div>
          <div>workforce comms · approvals · delivery ops</div>
        </div>
      </div>
      <div class="herorow">
        <section class="hero">
          <div class="chiprow">
            ...
            <span class="meta-chip">FirstUp employee-comms proof</span>
            <span class="meta-chip">Synthetic campaign + packet exports</span>
          </div>
          <h1>Employee comms that stay review-safe.</h1>
          <p>This control plane turns FirstUp-flavored campaign and review data into one buyer-readable surface: missing audience evidence, approval proof drift, localization gaps, send-window blockers, and the exact packet sequence needed before an internal communication, policy push, or executive update goes wide.</p>
          <div class="navrow">
        ...
        <aside class="side">
          <div class="bluf">
            ...
            <p><strong>Employee communications evidence for internal comms, HR technology, security enablement, and policy-governance teams.</strong><br />Audit-safe visibility into missing audience proof, stale approvals, localization drift, and send posture without exposing live employee records.</p>
          </div>
          <div class="corr">
            <div class="lbl">Proof Layer</div>
            <p><strong>Offline analyzer + CLI + dashboard surface.</strong><br />This repo reads campaign and review snapshots and turns them into lane, gap, and send packets operators can actually use.</p>
          </div>
          <div class="corr">
            <div class="lbl">Why it matters</div>
            <p>Recruiters looking for <strong>FirstUp / internal communications / employee experience / audience governance / send operations</strong> should see real operator work: campaign evidence, approval posture, delivery safety, and owner-safe escalation.</p>
          </div>
        </aside>
      </div>
      ${body}
      <div class="footer">
        <div>firstup-employee-comm-audit · synthetic sample data only</div>
        <div>routes: / · /comms-lane · /message-gaps · /send-posture · /verification · /docs</div>
      </div>
    </div>
  </body>
</html>`;
...
export function renderOverview() {
  ...
  return layout(
    ...
    `<section class="section">
        <div class="sh"><h2>Operator Snapshot</h2><div class="note">send pressure · routing health · campaign posture</div></div>
        <div class="kpis">
          <div class="kpi cyan"><div class="v">${metrics.programs}</div><div class="lbl">campaigns</div><div class="h">Synthetic employee-communications launches modeled through audience evidence and send pressure.</div></div>
          <div class="kpi green"><div class="v">${metrics.onTrackPrograms}</div><div class="lbl">on-track campaigns</div><div class="h">Launches currently carrying healthy packet and review posture.</div></div>
          <div class="kpi plum"><div class="v">${metrics.packets}</div><div class="lbl">packets</div><div class="h">Evidence packets across audience, approval, localization, and delivery domains.</div></div>
          ...
          <div class="kpi amber"><div class="v">${metrics.workflowGaps}</div><div class="lbl">workflow gaps</div><div class="h">Campaign routes still missing healthy owner or escalation sequencing.</div></div>
          <div class="kpi red"><div class="v">${metrics.stalePackets}</div><div class="lbl">stale packets</div><div class="h">Open evidence packets older than the employee-comms review SLA.</div></div>
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>Why operators care</h2><div class="note">campaign trust · packet routing · send prevention</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">containment first</div><div class="src-tit">Route the packet before send posture hardens</div><p>${metrics.recommendation}</p></div>
          <div class="src"><div class="src-name">operator evidence</div><div class="src-tit">Turn employee comms exports into review proof</div><p>Every lane stays tied to owner, campaign, packet readiness, and the next concrete operator move.</p></div>
          <div class="src"><div class="src-name">recruiter signal</div><div class="src-tit">Show real internal-comms governance depth</div><p>This is real audience evidence and send-readiness routing, not generic employee-experience copy.</p></div>
        </div>
      </section>`
  );
}

export function renderCommsLane() {
  return layout(
    "FirstUp Employee Comm Audit — Comms Lane",
    "/comms-lane",
    `<section class="section">
        <div class="sh"><h2>Comms Lane</h2><div class="note">owner · focus · next action</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Lane</th>
...
export function renderMessageGaps() {
  return layout(
    "FirstUp Employee Comm Audit — Message Gaps",
    "/message-gaps",
    `<section class="section">
        <div class="sh"><h2>Message Gaps</h2><div class="note">severity · owner · packet</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Gap</th>
...
export function renderSendPosture() {
  return layout(
    "FirstUp Employee Comm Audit — Send Posture",
    "/send-posture",
    `<section class="section">
        <div class="sh"><h2>Send Posture</h2><div class="note">packet readiness · blocker · send window</div></div>
        <div class="board">
          ${sendPosture().map((packet) => `<article class="pcard">
              <div class="ptop">
...
export function renderValidation() {
  return layout(
    "FirstUp Employee Comm Audit — Verification",
    "/verification",
    `<section class="section">
        <div class="sh"><h2>Verification</h2><div class="note">operator-safe claims only</div></div>
        <div class="stack">
          ${verification().map((item, index) => `<div class="src"><div class="src-name">verification ${index + 1}</div><div class="src-tit">${item}</div><p>This surface stays explicit about offline exports, synthetic sample data, and real employee-communications posture.</p></div>`).join("")}
        </div>
      </section>`
  );
}
...
export function renderDocs() {
  return layout(
    "FirstUp Employee Comm Audit — Docs",
    "/docs",
    `<section class="section">
        <div class="sh"><h2>Docs</h2><div class="note">routes · cli · api</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">routes</div><div class="src-tit">Public control surface</div><p><code>/</code>, <code>/comms-lane</code>, <code>/message-gaps</code>, <code>/send-posture</code>, <code>/verification</code>, <code>/docs</code></p></div>
          <div class="src"><div class="src-name">api</div><div class="src-tit">Structured payloads</div><p><code>/api/dashboard/summary</code>, <code>/api/comms-lane</code>, <code>/api/message-gaps</code>, <code>/api/send-posture</code>, <code>/api/verification</code>, <code>/api/sample</code></p></div>
          <div class="src"><div class="src-name">cli</div><div class="src-tit">Offline comms analysis</div><p><code>npx firstup-comm-audit fixtures/firstup-employee-comms-clean.json --format summary</code> renders the same send posture the dashboard exposes.</p></div>
        </div>
      </section>`
  );
}
