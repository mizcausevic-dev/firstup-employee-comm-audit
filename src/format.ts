...
export function toSummary(report: CoverageReport) {
  ...
  return `${posture} ${report.programs} campaigns · ${report.packets} packets · ${report.highSeverityPackets} high-severity packets · ${report.workflowGaps} workflow gaps`;
}
...
export function toMarkdown(report: CoverageReport) {
  ...
  return [
    `# Employee comms posture ${report.ok ? "✅" : "❌"}`,
    "",
    `- Campaigns: **${report.programs}**`,
    `- On-track campaigns: **${report.onTrackPrograms}**`,
